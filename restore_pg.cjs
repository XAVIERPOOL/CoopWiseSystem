...
async function restore() {
  ...
  try {
    console.log('Restoring records with pg...');
    await client.query('BEGIN');
    
    // Create accurate table to ensure no missing columns (reviewer_notes, etc)
    await client.query(`
    ...
    await client.query('DELETE FROM compliance_records');
    ...
    const countRes = await client.query('SELECT COUNT(*) FROM compliance_records');
    console.log('Successfully restored ' + countRes.rows[0].count + ' records!');
