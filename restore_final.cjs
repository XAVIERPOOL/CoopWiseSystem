...
async function run() {
  try {
    ...
    await sql`DROP TABLE IF EXISTS compliance_records`;
    ...
    console.log('Inserting 44 records...');
    for (const record of MOCK_RECORDS) {
      await sql`
        INSERT INTO compliance_records (cooperative_name, cooperative_type, requirement_name, status, submitted_date, deadline, reviewer_notes, file_url)
        VALUES (
          ${record.cooperative_name}, 
          ${record.cooperative_type || null}, 
          ${record.requirement_name}, 
          ${record.status}, 
          ${record.submitted_date || null}, 
    ...
    const count = await sql`SELECT COUNT(*) as count FROM compliance_records`;
    console.log('Done! Final count:', count[0].count);
